...
- **[arXiv251224] An Adaptive Distributed Stencil Abstraction for GPUs**
  - **tags:** [sys], [high-performance computing], [stencil computations, GPU, distributed abstraction, Charm++, NumPy-like syntax, resource elasticity]
  - **authors:** Aditya Bhosale, Laxmikant Kale
  - **institution:** University of Illinois at Urbana-Champaign
  - **link:** https://arxiv.org/pdf/2512.19851
  - **Simple LLM Summary:** The paper presents an adaptive, distributed abstraction for stencil computations on multi-node GPUs, built using the Charm++ runtime with a NumPy-like syntax. It demonstrates the system's resource elasticity through dynamic rescaling and shows significant performance improvements over specialized stencil DSLs and generalized NumPy replacements.
...
- **[arXiv251224] Reaching Agreement Among Reasoning LLM Agents**
  ...
  - **Simple LLM Summary:** This paper proposes Aegean, a formal consensus protocol for multi-agent LLM reasoning systems, implemented in the Aegean-Serve serving engine. It uses incremental quorum detection to enable early termination when agents converge, reducing latency. The evaluation shows it provides safety and liveness guarantees while significantly cutting latency without sacrificing answer quality.

- **[arXiv251224] SHIRO: Near-Optimal Communication Strategies for Distributed Sparse Matrix Multiplication**
  - **tags:** [mlsys], [cluster infrastructure], [sparsity-aware communication, hierarchical communication, distributed sparse matrix multiplication, GPU-accelerated systems]
  ...
  - **Simple LLM Summary:** The paper proposes SHIRO, a distributed framework that reduces communication overhead in sparse matrix multiplication by using a sparsity-aware communication strategy and a hierarchical strategy tailored for two-tier GPU cluster networks. It demonstrates strong scalability up to 128 GPUs, achieving significant speedups over state-of-the-art baselines.

- **[arXiv251224] Holoscope: Open and Lightweight Distributed Telescope & Honeypot Platform**
  - **tags:** [sys], [cybersecurity observatory], [K3s, WireGuard, Infrastructure-as-Code, container-based, darknet, honeypot]
  ...
  - **Simple LLM Summary:** Holoscope is a lightweight, cloud-native platform built on K3s and WireGuard for deploying and managing distributed network telescopes and honeypots. It simplifies secure, automated sensor orchestration across multiple networks and institutions. The platform has been successfully deployed across organizations in Europe and Brazil, providing unified visibility into large-scale attack phenomena.

- **[arXiv251224] Learned Digital Codes for Over-the-Air Computation in Federated Edge Learning**
  - **tags:** [mlsys], [others], [over-the-air computation, unsourced random access, vector quantization, approximate message passing, federated edge learning, digital aggregation]
  - **authors:** Antonio Tarizzo, Mohammad Kazemi, Deniz Gündüz
  - **institution:** Imperial College London
  - **link:** https://arxiv.org/pdf/2512.19777
  - **Simple LLM Summary:** This paper proposes a learned digital over-the-air computation framework for federated edge learning, integrating an unsourced random access codebook with a trained AMP-style decoder. The method improves recovery accuracy and robustness in low SNR regimes, extending reliable operation by more than 10 dB while supporting a broad class of symmetric aggregation functions.

- **[arXiv251224] FastMPS: Revisit Data Parallel in Large-scale Matrix Product State Sampling**
  - **tags:** [mlsys], [cluster infrastructure], [matrix product state, data parallelism, tensor parallelism, compression, overlapping, Gaussian Boson Sampling]
  ...
  - **Simple LLM Summary:** This paper proposes FastMPS, a multi-level parallel framework that combines data parallelism across samples with tensor parallelism along bond dimensions for scalable Matrix Product State sampling. It uses compression and overlapping techniques to reduce memory and I/O pressure. The method achieves over 10x speedup, scales to thousands of processes, and enables large-scale simulations, significantly outperforming existing simulators.

- **[arXiv251224] Population Protocols Revisited: Parity and Beyond**
  - **tags:** [sys], [distributed computing], [population protocols, parity computation, congruence predicates, weight system, clocking mechanism, anomaly detection, switching mechanism, silent stabilisation]
  ...
  - **Simple LLM Summary:** The paper introduces a new computing paradigm for population protocols that integrates population weights, a robust clocking mechanism, and anomaly detection with a switching mechanism. This approach yields the first efficient protocols for parity and congruence predicates, using O(log³ n) states and achieving silent stabilisation in O(log³ n) time. The method enables implicit conversion between unary and binary representations, facilitating universal design for efficient multistage protocols.

- **[arXiv251224] Scaling Point-based Differentiable Rendering for Large-scale Reconstruction**
  - **tags:** [mlsys], [others], [point-based differentiable rendering, distributed training, data locality, communication optimization, unified API]
  ...
  - **Simple LLM Summary:** This paper introduces Gaian, a general distributed training system for Point-based Differentiable Rendering (PBDR) that addresses communication overhead and poor data locality in existing systems. Gaian provides a unified API to support various PBDR methods and uses data-access information to optimize performance. The system reduces communication by up to 91% and improves training throughput by 1.50x-3.71x across multiple datasets and up to 128 GPUs.

- **[arXiv251224] UCCL-EP: Portable Expert-Parallel Communication**
  - **tags:** [mlsys], [llm training], [expert parallelism, GPUDirect RDMA, GPU-CPU control channel, RDMA immediate data, token-level communication]
  ...
  - **Simple LLM Summary:** UCCL-EP introduces a portable expert-parallel communication system that replaces GPU-initiated RDMA with a high-throughput GPU-CPU control channel and uses RDMA immediate data to emulate ordering semantics. This approach achieves performance comparable to DeepEP while improving portability across heterogeneous GPU and NIC platforms, such as AWS EFA and Broadcom NICs, and boosts throughput in both inference and training workloads.

- **[arXiv251224] Rethinking Knowledge Distillation in Collaborative Machine Learning: Memory, Knowledge, and Their Interactions**
  - **tags:** [mlsys], [others], [knowledge distillation, collaborative learning, federated learning, multi-agent domain adaptation, federated multi-modal learning, federated continual learning, federated multi-task learning, federated graph knowledge embedding]
  ...
  - **Simple LLM Summary:** This paper provides a comprehensive review of Knowledge Distillation (KD) within collaborative machine learning, focusing on the roles and interactions of memory and knowledge. It analyzes how knowledge is extracted, stored, and shared across various collaborative patterns like federated learning, examining challenges such as task, model, and data heterogeneity. The work concludes by categorizing existing approaches and proposing future directions to advance KD techniques in distributed, privacy-aware systems.

- **[arXiv251224] WOC: Dual-Path Weighted Object Consensus Made Efficient**
  - **tags:** [sys], [distributed consensus protocols], [weighted quorums, dual-path consensus, object-weighted consensus, leaderless fast path, node-weighted consensus]
  - **authors:** Tanisha Fonseca, Gengrui Zhang
  - **institution:** Concordia University
  - **link:** https://arxiv.org/pdf/2512.20485
  - **Simple LLM Summary:** WOC is a dual-path consensus protocol that dynamically routes operations: independent operations use a fast, leaderless path with object-specific weighted quorums, while conflicting operations use a leader-coordinated slow path with node-weighted consensus. It achieves up to 4x higher throughput than prior protocols like Cabinet for workloads with many independent objects, while maintaining performance under high contention.

- **[arXiv251224] Clust-PSI-PFL: A Population Stability Index Approach for Clustered Non-IID Personalized Federated Learning**
  - **tags:** [mlsys], [cluster infrastructure], [population stability index, clustering, k-means++, silhouette score, personalized federated learning, non-iid data]
  ...
  - **Simple LLM Summary:** The paper proposes Clust-PSI-PFL, a personalized federated learning framework that uses the Population Stability Index (PSI) to measure data distribution divergence among clients and then clusters them into homogeneous groups using K-means++. This clustering approach improves global model accuracy by up to 18% and enhances client fairness by 37% under severe non-IID data conditions.
...
- **[arXiv251224] Resilient Packet Forwarding: A Reinforcement Learning Approach to Routing in Gaussian Interconnected Networks with Clustered Faults**
  - **tags:** [mlsys], [fault-tolerance], [reinforcement learning, PPO (Proximal Policy Optimization), greedy adaptive routing, fault-aware routing]
  - **authors:** Mohammad Walid Charrwi, Zaid Hussain
  - **institution:** Kuwait University
  - **link:** https://arxiv.org/pdf/2512.20394
  - **Simple LLM Summary:** This paper proposes a fault-aware routing scheme for Gaussian Interconnected Networks using a Reinforcement Learning agent based on Proximal Policy Optimization (PPO). The RL agent is trained with a reward structure that penalizes proximity to faulty nodes, enabling it to dynamically learn to bypass faulty regions. The results show that the RL-based approach significantly outperforms a greedy adaptive shortest-path routing algorithm in maintaining high packet delivery ratios under high fault densities and low network loads.

- **[arXiv251224] Predictive-LoRA: A Proactive and Fragmentation-Aware Serverless Inference System for LLMs**
  - **tags:** [mlsys], [llm inference], [Low-Rank Adaptation (LoRA), LSTM-based traffic predictor, page-based memory management, virtual memory, prefetching, fragmentation-aware]
  ...
  - **Simple LLM Summary:** The paper presents Predictive-LoRA (P-LoRA), a serverless inference system that uses an LSTM-based traffic predictor to proactively prefetch adapters and a page-based memory management mechanism to reduce fragmentation. It demonstrates that P-LoRA significantly reduces cold start latency and improves throughput compared to existing systems like S-LoRA.

- **[arXiv251224] Fail Fast, Win Big: Rethinking the Drafting Strategy in Speculative Decoding via Diffusion LLMs**
  - **tags:** [mlsys], [llm inference], [speculative decoding, diffusion large language models, parallel token generation, dynamic speculation length, autoregressive verification]
  - **authors:** Rui Pan, Zhuofu Chen, Ravi Netravali
  - **institution:** Princeton University
  - **link:** https://arxiv.org/pdf/2512.20573
  - **Simple LLM Summary:** The paper introduces FailFast, a speculative decoding framework that uses a diffusion LLM as a fast, parallel drafter to generate long draft sequences, which are then verified by an autoregressive LLM. By dynamically adapting the draft length, it minimizes compute in difficult regions and aggressively extends drafts in easy regions, achieving significant speedups. The method provides lossless acceleration, achieving up to 4.9x speedup over vanilla decoding without any fine-tuning.
...
- **[arXiv251225] ESCHER: Efficient and Scalable Hypergraph Evolution Representation with Application to Triad Counting**
  - **tags:** [sys], [graph and hypergraph data structures], [GPU-centric parallel data structure, dynamic hypergraph representation, triad counting]
  ...
  - **Simple LLM Summary:** The paper proposes ESCHER, a GPU-centric parallel data structure designed for efficient and scalable representation of dynamic hypergraphs. It also introduces a triad-count update framework that minimizes redundant computation. The method significantly outperforms existing approaches, achieving speedups of up to several hundred times faster for different types of hypergraph triad counting.

- **[arXiv251225] RHAPSODY: Execution of Hybrid AI-HPC Workflows at Scale**
  - **tags:** [mlsys], [cluster infrastructure], [multi-runtime middleware, uniform abstractions, Dragon, vLLM, heterogeneous workflows, AI-HPC coupling]
  ...
  - **Simple LLM Summary:** The paper presents RHAPSODY, a multi-runtime middleware that coordinates existing runtimes to enable the concurrent execution of heterogeneous AI-HPC workloads (like simulations and inference services) within a single job. The evaluation shows that RHAPSODY introduces minimal overhead, scales well for inference workloads, and provides efficient coupling between AI and HPC tasks.

- **[arXiv251225] SoK: Speedy Secure Finality**
  - **tags:** [sys], [blockchain consensus protocols], [Gasper, Speedy Secure Finality, reorganization resilience, generalized sleepy model, Goldfish, RLMD-GHOST, Single Slot Finality, 3-Slot Finality]
  ...
  - **Simple LLM Summary:** This paper surveys protocols for achieving fast transaction finality in blockchain networks like Ethereum. It traces the development of theoretical primitives and analyzes practical bottlenecks, concluding that the 3-Slot Finality protocol offers a practical balance between speed and the engineering constraints of large validator sets.

- **[arXiv251225] AirGS: Real-Time 4D Gaussian Streaming for Free-Viewpoint Video Experiences**
  - **tags:** [mlsys], [others], [4D Gaussian Splatting, keyframe identification, integer linear programming, pruning level selection, temporal coherence, inflation loss]
  - **authors:** Zhe Wang, Jinghang Li, Yifei Zhu
  - **institution:** Shanghai Jiao Tong University
  - **link:** https://arxiv.org/pdf/2512.20943
  - **Simple LLM Summary:** AirGS is a streaming-optimized 4D Gaussian Splatting framework that converts Gaussian video streams into multi-channel 2D formats, uses keyframes and an inflation loss to improve quality and reduce size, and adaptively prunes updates for efficient transmission. It significantly reduces quality deviation, accelerates training, and cuts transmission size compared to state-of-the-art methods.

- **[arXiv251225] Stochastic well-structured transition systems**
  - **tags:** [sys], [distributed computing], [well-structured transition systems, stochastic scheduling, population protocols, phase clock, computational power characterization]
  - **authors:** James Aspnes
  - **institution:** Yale University
  - **link:** https://arxiv.org/pdf/2512.20939
  - **Simple LLM Summary:** This paper introduces stochastic well-structured transition systems (SWSTSs), which extend classical well-structured transition systems by incorporating a probabilistic scheduling rule. It shows that phase clocks in these systems have polynomial expected duration and that terminating computations finish in expected polynomial time. This leads to a characterization of computational power, showing that augmented systems compute exactly the languages in BPP, while unaugmented ones compute symmetric languages in BPL.

- **[arXiv251225] Deadline-Aware Online Scheduling for LLM Fine-Tuning with Spot Market Predictions**
  - **tags:** [mlsys], [llm training], [online scheduling, spot instances, on-demand instances, integer programming, committed horizon control, online policy selection, regret bound]
  ...
  - **Simple LLM Summary:** This paper proposes an online scheduling framework that mixes spot and on-demand GPU instances for cost-efficient LLM fine-tuning under deadlines. It introduces a prediction-based algorithm, a complementary algorithm without predictions, and a policy selection algorithm that adaptively chooses the best one. Experimental results show the framework consistently outperforms baselines, improving utility by up to 54.8%.

- **[arXiv251225] Mesh-Attention: A New Communication-Efficient Distributed Attention with Improved Data Locality**
  - **tags:** [mlsys], [llm training], [distributed attention, ring-attention, communication-computation ratio, tile scheduling, greedy algorithm]
  ...
  - **Simple LLM Summary:** This paper proposes Mesh-Attention, a new distributed attention algorithm that assigns two-dimensional computation tiles to GPUs to reduce communication overhead compared to prior methods like Ring-Attention. It introduces a matrix-based model and a greedy scheduling algorithm to optimize the communication-computation ratio. Experiments show Mesh-Attention achieves significant speedups and communication reductions, demonstrating superior scalability for large-context LLM training.
...
- **[arXiv251225] zkFL-Health: Blockchain-Enabled Zero-Knowledge Federated Learning for Medical AI Privacy**
  ...
  - **Simple LLM Summary:** The paper proposes zkFL-Health, a framework that combines federated learning with zero-knowledge proofs and trusted execution environments to enable verifiable and privacy-preserving collaborative training for medical AI. It uses a blockchain to record cryptographic commitments, ensuring integrity and auditability without revealing client updates. The main conclusion is that this architecture provides strong confidentiality, integrity, and auditability, which are crucial for clinical adoption and regulatory compliance.

- **[arXiv251225] Diving into 3D Parallelism with Heterogeneous Spot Instance GPUs: Design and Implications**
  - **tags:** [mlsys], [llm training], [3D parallelism, heterogeneous GPUs, AutoHet, spot instances, load balancing, asymmetric pipeline parallelism, tensor parallelism, data parallelism]
  ...
  - **Simple LLM Summary:** This paper introduces AutoHet, a system that automatically finds the optimal 3D parallelism plan for distributed training of large models across heterogeneous GPUs, addressing challenges like asymmetric pipeline parallelism and load balancing. It includes an efficient recovery strategy for spot instance preemptions. Evaluations show AutoHet achieves up to a 1.79x training speedup over existing systems and a 4.38x faster recovery.

- **[arXiv251225] PHOTON: Hierarchical Autoregressive Modeling for Lightspeed and Memory-Efficient Language Generation**
  - **tags:** [mlsys], [llm inference], [hierarchical autoregressive modeling, KV-cache reduction, multi-resolution context access, parallel hierarchical operation, bottom-up encoder, top-down decoder]
  ...
  - **Simple LLM Summary:** PHOTON introduces a hierarchical autoregressive model that replaces flat token-by-token scanning with vertical, multi-resolution context access, using a bottom-up encoder to compress tokens into low-rate states and lightweight top-down decoders to reconstruct token representations. This approach significantly reduces decode-time KV-cache traffic, improving throughput per unit memory by up to 10^3× and offering advantages in long-context and multi-query tasks compared to Transformer-based models.

- **[arXiv251225] Declarative distributed broadcast using three-valued modal logic and semitopologies**
  - **tags:** [sys], [distributed algorithms], [modal logic, three-valued logic, semitopologies, axiomatic specification, declarative protocols, Bracha Broadcast, Crusader Agreement]
  - **authors:** Murdoch J. Gabbay
  - **institution:** Heriot-Watt University
  - **link:** https://arxiv.org/pdf/2512.21137
  - **Simple LLM Summary:** The paper proposes a method for formally specifying distributed algorithms as declarative axiomatic theories using three-valued modal logic and semitopologies. This approach abstracts away low-level implementation details to capture the logical essence of protocols like voting, broadcast, and agreement. The method provides a precise, human-readable foundation for verification and has been used to find errors in an industrial protocol.
