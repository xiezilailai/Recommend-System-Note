...
- **[arXiv260227] Energy Efficient Federated Learning with Hyperdimensional Computing (HDC)**
  - **tags:** [mlsys], [others], [hyperdimensional computing, differential privacy, federated learning, resource allocation, energy efficiency]
  ...
  - **Simple LLM Summary:** The paper proposes a federated learning framework that uses hyperdimensional computing for lightweight local training and applies differential privacy to secure model updates. It minimizes total energy consumption by jointly optimizing the HDC dimension, transmit power, and CPU frequency. The method achieves significant energy reduction while maintaining model accuracy and faster convergence.

- **[arXiv260227] Fault-tolerant Reduce and Allreduce operations based on correction**
  - **tags:** [sys], [fault-tolerance], [reduce, allreduce, broadcast, gossip, tree-based communication, correction algorithm, collective communication, HPC]
  - **authors:** Martin Kuettler, Hermann Haertig
  - **institution:** TU Dresden
  - **link:** https://arxiv.org/pdf/2602.22445
  - **Simple LLM Summary:** This paper proposes a fault-tolerant algorithm for Reduce and Allreduce operations by using a correction-like communication phase followed by a tree-based phase, similar to prior work on Broadcast. It provides and proves the semantics of the resulting algorithm, which is designed to tolerate process failures in high-performance computing environments.

- **[arXiv260227] AdapTBF: Decentralized Bandwidth Control via Adaptive Token Borrowing for HPC Storage**
  - **tags:** [sys], [HPC storage systems], [Token Bucket Filter, adaptive borrowing, decentralized control, Lustre, bandwidth control]
  - **authors:** Md Hasanur Rashid, Dong Dai
  - **institution:** University of Delaware
  - **link:** https://arxiv.org/pdf/2602.22409
  - **Simple LLM Summary:** The paper proposes AdapTBF, a decentralized bandwidth control method for HPC storage that enhances the Token Bucket Filter with an adaptive token borrowing and lending mechanism. It is implemented in the Lustre file system and evaluated with synthetic workloads. The results show that AdapTBF effectively manages I/O bandwidth to improve fairness and maintain high storage utilization under extreme conditions.

- **[arXiv260227] DIAL: Decentralized I/O AutoTuning via Learned Client-side Local Metrics for Parallel File System**
  - **tags:** [mlsys], [others], [decentralized autotuning, client-side local metrics, machine learning, parallel file systems, high-performance computing]
  ...
  - **Simple LLM Summary:** The paper proposes DIAL, a decentralized I/O autotuning framework for parallel file systems that uses machine learning models on each client to make configuration decisions based solely on locally observable metrics, avoiding the overhead of global pattern analysis. It concludes that this approach enables timely, fine-grained tuning and achieves better global I/O performance for applications.
...
- **[arXiv260227] From Prompts to Performance: Evaluating LLMs for Task-based Parallel Code Generation**
  ...
  - **Simple LLM Summary:** This paper evaluates how Large Language Models generate task-based parallel code from different prompts (natural language, sequential code, pseudo code) across three frameworks (OpenMP, C++ std, HPX). The results show that LLMs have varying strengths and weaknesses depending on the problem complexity and framework, indicating challenges for reliable LLM-assisted development in high-performance computing.

- **[arXiv260227] GetBatch: Distributed Multi-Object Retrieval for ML Data Loading**
  - **tags:** [mlsys], [cluster infrastructure], [GetBatch API, object store, batched retrieval, distributed execution, streaming, fault-tolerance]
  - **authors:** Alex Aizman, Abhishek Gaikwad, Piotr Żelasko
  - **institution:** NVIDIA
  - **link:** https://arxiv.org/pdf/2602.22434
  - **Simple LLM Summary:** The paper introduces GetBatch, a new object store API that replaces thousands of individual GET requests for ML data loading with a single, deterministic, and fault-tolerant batch retrieval operation. This method combines the sampling flexibility of random access with the throughput of sequential I/O. The evaluation shows significant performance improvements, including up to 15x throughput gain and reduced latency in production training workloads.
...
- **[arXiv260227] veScale-FSDP: Flexible and High-Performance FSDP at Scale**
  ...
  - **Simple LLM Summary:** The paper introduces veScale-FSDP, a redesigned FSDP system that uses a flexible sharding format called RaggedShard and a structure-aware planning algorithm to support block-wise quantization and non-element-wise optimizers. It achieves higher throughput and lower memory usage than existing FSDP systems while scaling efficiently to tens of thousands of GPUs.

- **[arXiv260227] CARAT: Client-Side Adaptive RPC and Cache Co-Tuning for Parallel File Systems**
  - **tags:** [mlsys], [cluster infrastructure], [autotuning, parallel file systems, RPC tuning, cache co-tuning, machine learning, online adaptation, client-side tuning, Lustre]
  ...
  - **Simple LLM Summary:** CARAT is an ML-guided framework that co-tunes client-side RPC and caching parameters for parallel file systems online, using only locally observable metrics to adapt to dynamic I/O patterns and system states. It achieves up to 3x performance improvement over default configurations, demonstrating effective and scalable tuning for HPC workloads.

- **[arXiv260227] CCCL: Node-Spanning GPU Collectives with CXL Memory Pooling**
  - **tags:** [mlsys], [llm training], [CXL, collective communication, shared memory pool, GPU, NCCL, RDMA, InfiniBand]
  ...
  - **Simple LLM Summary:** The paper proposes CCCL, a collective communication library that leverages a CXL-based shared memory pool to enable efficient cross-node GPU operations, eliminating the need for traditional RDMA networking. It demonstrates performance improvements for operations like AllGather and Broadcast compared to InfiniBand, showing CXL's potential for scalable and cost-effective communication in large-scale LLM training.
...
- **[arXiv260227] Engineered Simultaneity: The Physical Impossibility of Consolidated Price Discovery Across Spacelike-Separated Exchanges**
  - **tags:** [sys], [distributed systems, financial regulation], [engineered simultaneity, special relativity, NBBO, Regulation NMS, Securities Information Processor, latency arbitrage, causal precedence, Lamport's theorem]
  - **authors:** Paul Borrill
  - **institution:** DAEDAELUS
  - **link:** https://arxiv.org/pdf/2602.22350
  - **Simple LLM Summary:** The paper applies concepts from special relativity and distributed systems theory to analyze the U.S. National Best Bid and Offer (NBBO). It proves that the NBBO's requirement for a consolidated price across spacelike-separated exchanges is physically impossible, as simultaneity is frame-dependent, creating a latency arbitrage opportunity exploited by high-frequency traders. The main conclusion is that this "engineered simultaneity" constitutes a category mistake, extracting approximately $5 billion annually from other market participants.

- **[arXiv260227] FLYING SERVING: On-the-Fly Parallelism Switching for Large Language Model Serving**
  - **tags:** [mlsys], [llm inference], [data parallelism, tensor parallelism, vLLM, KV cache, dynamic reconfiguration, online switching]
  ...
  - **Simple LLM Summary:** The paper presents Flying Serving, a system that enables on-the-fly switching between data and tensor parallelism for LLM serving without restarting workers. It achieves this through components like a zero-copy Model Weights Manager and a KV Cache Adaptor. The system significantly improves performance under varying loads while supporting mixed request requirements.
...
- **[arXiv260227] FuxiShuffle: An Adaptive and Resilient Shuffle Service for Distributed Data Processing on Alibaba Cloud**
  ...
  - **institution:** Wuhan University, Alibaba Cloud, Alibaba Group, The Chinese University of Hong Kong
  - **link:** https://arxiv.org/pdf/2602.22580
  - **Simple LLM Summary:** The paper presents FuxiShuffle, an adaptive and resilient shuffle service designed for Alibaba Cloud's MaxCompute platform. It dynamically selects shuffle modes, schedules downstream workers based on progress, and uses multi-replica failover with incremental recovery to handle failures efficiently. Experiments show it reduces job completion time and aggregate resource consumption compared to baseline systems.

- **[arXiv260227] Tackling Privacy Heterogeneity in Differentially Private Federated Learning**
  - **tags:** [mlsys], [others], [differential privacy, federated learning, client selection, privacy heterogeneity, convex optimization, convergence analysis]
  - **authors:** Ruichen Xu, Ying-Jun Angela Zhang, Jianwei Huang
  - **institution:** The Chinese University of Hong Kong, Shenzhen Institute of Artificial Intelligence and Robotics for Society
  - **link:** https://arxiv.org/pdf/2602.22633
  - **Simple LLM Summary:** The paper proposes a privacy-aware client selection strategy for differentially private federated learning, formulated as a convex optimization problem to minimize training error under heterogeneous privacy budgets. It demonstrates that this approach improves test accuracy by up to 10% on CIFAR-10 compared to baselines, highlighting the importance of adapting client selection to varying privacy requirements.
...
- **[arXiv260227] Accelerating Local LLMs on Resource-Constrained Edge Devices via Distributed Prompt Caching**
  ...
  - **Simple LLM Summary:** This paper proposes a distributed prompt caching method to accelerate local LLM inference on resource-constrained edge devices by sharing intermediate processing states (KV cache) across devices. It uses a Bloom-filter-based catalog to reduce communication overhead for state sharing. Experiments on a Raspberry Pi platform show significant reductions in Time to First Token (93.12%) and Time to Last Token (50.07%).
...
- **[arXiv260227] Dynamic Hierarchical Birkhoff-von Neumann Decomposition for All-to-All GPU Communication**
  ...
  - **Simple LLM Summary:** The paper proposes a dynamic hierarchical Birkhoff-von Neumann decomposition framework to schedule all-to-all communication in two-tier GPU clusters. It balances traffic within servers and then applies a hierarchical decomposition to reduce complexity. Simulations show the method reduces mean frame length, especially under skewed traffic.
...
- **[arXiv260227] An Artificial Intelligence Framework for Joint Structural-Temporal Load Forecasting in Cloud Native Platforms**
  ...
  - **Simple LLM Summary:** The paper proposes a joint structural-temporal load forecasting framework for cloud native platforms, integrating service invocation graphs with multivariate load sequences and using attention mechanisms enhanced by structural priors. It employs multi-objective regression to optimize predictions across service and cluster levels, improving cross-granularity stability. The framework supports capacity assessment and resource orchestration in dynamic cloud environments.

- **[arXiv260227] Distributed LLM Pretraining During Renewable Curtailment Windows: A Feasibility Study**
  - **tags:** [mlsys], [llm training], [federated learning, curtailment-aware scheduling, geo-distributed GPU clusters, transformer model]
  ...
  - **Simple LLM Summary:** The paper proposes a system that performs distributed LLM pretraining across geo-distributed GPU clusters during renewable energy curtailment windows, using federated learning to elastically switch between local and multi-site training based on site availability. Preliminary results show this curtailment-aware scheduling approach maintains training quality while reducing operational carbon emissions to 5–12% of single-site baselines.

- **[arXiv260227] Workload Buoyancy: Keeping Apps Afloat by Identifying Shared Resource Bottlenecks**
  - **tags:** [sys], [resource management], [buoyancy, bottleneck detection, shared resource contention, performance metrics, workload orchestration]
  - **authors:** Oliver Larsson, Thijs Metsch, Cristian Klein, Erik Elmroth
  - **institution:** Umeå University
  - **link:** https://arxiv.org/pdf/2602.22852
  - **Simple LLM Summary:** This paper introduces "buoyancy," a novel abstraction that integrates application-level and system-level metrics to characterize workload performance and identify shared resource bottlenecks in multi-tenant systems. The method provides a more holistic view of performance dynamics compared to traditional heuristics. The evaluation shows that buoyancy offers a 19.3% better indication of bottlenecks on average, enabling improved observability and scheduling decisions.

- **[arXiv260227] RLHFless: Serverless Computing for Efficient RLHF**
  - **tags:** [mlsys], [llm training], [serverless computing, RLHF, synchronous training, cost-aware actor scaling, prefix pre-computation, workload assignment]
  ...
  - **Simple LLM Summary:** The paper presents RLHFless, a scalable training framework for synchronous Reinforcement Learning from Human Feedback (RLHF) built on serverless computing. It adapts to dynamic resource demands, pre-computes shared prefixes, and uses a cost-aware scaling strategy to improve efficiency. Experiments show RLHFless achieves up to 1.35x speedup and 44.8% cost reduction compared to state-of-the-art baselines.

- **[arXiv260227] A Simple Distributed Deterministic Planar Separator**
  - **tags:** [sys], [distributed algorithms], [planar separator, deterministic algorithm, weight transfer, diameter, Lipton-Tarjan]
  - **authors:** Yaseen Abd-Elhaleem, Michal Dory, Oren Weimann
  - **institution:** University of Haifa
  - **link:** https://arxiv.org/pdf/2602.22916
  - **Simple LLM Summary:** The paper presents a simple deterministic distributed algorithm for finding a balanced separator of size O(D) in planar graphs, where D is the diameter. The key innovation is a straightforward method where each vertex transfers its weight to an arbitrary adjacent face, simplifying previous complex or randomized approaches. This yields a near-optimal round complexity of Õ(D) and directly derandomizes several state-of-the-art distributed algorithms for planar graph problems.

- **[arXiv260227] LLMServingSim 2.0: A Unified Simulator for Heterogeneous and Disaggregated LLM Serving Infrastructure**
  - **tags:** [mlsys], [llm inference], [system-level simulator, heterogeneous hardware, disaggregated serving, runtime-driven modeling, profile-based modeling, batching, routing, offloading, power modeling]
  - **authors:** Jaehong Cho, Hyunmin Choi, Guseul Heo, Jongse Park
  - **institution:** KAIST
  - **link:** https://arxiv.org/pdf/2602.23036
  - **Simple LLM Summary:** This paper introduces LLMServingSim 2.0, a unified simulator that explicitly models the runtime interactions between heterogeneous hardware and disaggregated software in LLM serving infrastructures. It embeds serving decisions and hardware behavior into a single runtime loop to enable interaction-aware analysis. The simulator is validated to accurately reproduce performance metrics with low error, providing a practical tool for co-designing next-generation LLM serving systems.

- **[arXiv260227] Exploiting network topology in brain-scale simulations of spiking neural networks**
  - **tags:** [mlsys], [cluster infrastructure], [spiking neural networks, collective communication, synchronization, network topology, local-global hybrid communication, performance profiling]
  ...
  - **Simple LLM Summary:** This paper proposes a structure-aware mapping strategy for brain-scale spiking neural network simulations, exploiting the biological organization of the brain into areas to partition communication into frequent local and infrequent global exchanges. This local-global hybrid communication architecture reduces synchronization overhead and waiting time for the slowest compute node, challenging the belief that interconnect speed is the primary bottleneck. The method demonstrates substantial performance gains and provides guidelines for energy-efficient simulation on conventional supercomputers.

- **[arXiv260227] STELLAR: Storage Tuning Engine Leveraging LLM Autonomous Reasoning for High Performance Parallel File Systems**
  - **tags:** [mlsys], [others], [large language models, retrieval-augmented generation, multiagent design, autonomous tuning, parallel file systems]
  ...
  - **Simple LLM Summary:** This paper proposes STELLAR, an autonomous storage tuner that uses large language models (LLMs) to perform end-to-end agentic reasoning for optimizing parallel file systems. It integrates techniques like retrieval-augmented generation (RAG) and a multiagent design to extract parameters, analyze logs, and iteratively tune configurations. The evaluation shows STELLAR can find near-optimal configurations within just a few attempts, making I/O performance tuning more accessible and efficient.
...
- **[arXiv260227] A High-Throughput AES-GCM Implementation on GPUs for Secure, Policy-Based Access to Massive Astronomical Catalogs**
  - **tags:** [sys], [high-performance computing], [GPU acceleration, AES-GCM, parallel tree-reduction, GHASH, authenticated encryption, policy engine]
  ...
  - **Simple LLM Summary:** This paper presents a framework that combines a policy engine for access control with a GPU-accelerated AES-GCM implementation. The core method uses a parallel tree-reduction strategy to overcome the sequential bottleneck of the GHASH authentication, enabling high-throughput encryption. The main conclusion is that this approach provides secure, policy-based access to massive astronomical catalogs, ensuring data confidentiality and integrity without hindering performance for petabyte-scale analysis.
